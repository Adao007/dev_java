...
public class RSLinkedList implements LinkedListInterface {
	...
	private Flight<E> head; 		// Reference to the head of the linked list
	...
	public void add(int index, E item) {
		...
		if(index < 0 || index > size) { // checking for a valid index
			...
			Flight<E> node = getNode(index); // Get the reference of the previous node
			addAfter(node, item); 
		}
	}
	
	private void addAfter(Flight<E> node, E item) {
		node.next = new Flight<E>(item, node.next);
		size++; 
		
	}
	
	private Flight<E> getNode(int index) {
		...
		Flight<E> node = head; // this is a reference copy of the head
	...
	private void addFirst(E item) {
		...
		head.next = new Flight<E>(item, head.next); 
		size++; 
	}
	...
	public E remove(int index) {
		...
		if(index < 0 || index > size) { // checking for a valid index
			...
			Flight<E> node = getNode(index); 
			return removeAfter(node);
		}
	}

	private E removeAfter(Flight<E> node) {
		Flight<E> temp = node.next; 
		if(temp != null) {
			node.next = temp.next; 
			size--;
			return temp.data; 
	...
	public String toString() {
		String s = "["; 
		Flight<E> p = head;  // This reference will be used to iterate over the Linked List
